...
| Latency "< 50 ms per frame" | **REAL/MEASURED (report the true number)** | `eval.py` times the real per-frame cost on this machine and prints it. Report *that* number, whatever it is — do not assert 50 ms as a target hit unless the measurement says so. |
...
| ">90% Pearson correlation with UPDRS" | **REAL (synthetic data ONLY)** | `eval.py` reports correlation between predicted severity and *synthetic* ground-truth severity. This is a method demo, NOT a clinical correlation. There is no real UPDRS correlation because CARE-PD was never trained on here. |
...
| "Fine-tuned on CARE-PD, 8000+ sequences" | **ASPIRATIONAL** | CARE-PD is gated and not on this machine. `carepd.py` is a ready-to-run loader; it raises a clear error instead of fabricating data. No fine-tuning was performed. |
