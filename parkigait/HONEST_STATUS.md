...
| ">90% Pearson correlation with UPDRS" | **REAL, MEASURED — ~0.70, NOT >0.90** | Trained on **real CARE-PD UPDRS-gait labels** (25 clinical 3D features, subject-level CV): held-out **r ≈ 0.70, r² ≈ 0.49 pooled (0.75 / r² 0.57 best cohort); subject-aggregated r ≈ 0.76–0.78**. Verified leakage-free (permutation r ≈ 0.05; independent adversarial audit; cohort confound ruled out). Plateaus at ~0.70 — that's the honest ceiling, NOT the poster's >0.90. See [CAREPD_RESULTS.md](CAREPD_RESULTS.md). |
...
| Abnormal-gait detection on CARE-PD | **REAL, MEASURED — AUC ≈ 0.86–0.87** | Detect UPDRS-gait > 0: pooled **AUC 0.86–0.87** (calibrated ensemble), ~79% sens/spec, subject-level CV, permutation p<0.005. A gait-screening number, not standalone PD diagnosis. See [CAREPD_RESULTS.md](CAREPD_RESULTS.md). |
| External validation (unseen hospital) | **REAL, MEASURED — AUC 0.77** | Leave-one-cohort-out (train 3 sites, test the 4th): pooled **AUC 0.77** (0.74–0.86 by site). The realistic new-clinic number, stricter than CV. See [CLINICAL_PLUS.md](CLINICAL_PLUS.md). |
| Severity staging agreement | **REAL, MEASURED — QWK 0.62** | UPDRS-gait 0–3 quadratic weighted kappa **0.62** ("substantial"), 95% within one level, test-retest ICC 0.63. See [CLINICAL_PLUS.md](CLINICAL_PLUS.md). |
| "Fine-tuned on CARE-PD, 8000+ sequences" | **DONE (real data, ~2953 labelled walks)** | CARE-PD is downloaded and the model is trained on it: 110 UPDRS-labelled subjects, ~2953 walks, subject-level CV (`python -m parkigait carepd-rich`). Note it is ~2953 *labelled* walks (not 8000+), joints are canonical-FK approximations (not licensed SMPL). |
