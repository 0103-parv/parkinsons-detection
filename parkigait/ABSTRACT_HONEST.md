...
validation: for **abnormal-gait detection** (the screening task) the model reached
**AUC ≈ 0.86** (0.87 best cohort, ~79% sensitivity/specificity); for the harder
severity regression, a held-out correlation with clinician-rated UPDRS-gait of
**r ≈ 0.70 (r² ≈ 0.49)**, 0.75 on the largest cohort (0.76–0.78 per-subject). Both
were verified leakage-free by permutation controls (detection AUC 0.51, regression
r 0.05) and an independent adversarial audit — strong, honest results, with the
severity regression still below the synthetic method-demo (r ≈ 0.99) and any ">0.90". Pose extraction ran at **~27 ms/frame** on CPU
